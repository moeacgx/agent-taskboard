...
function parseTaskMerge(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["operationId", "projectId", "sourceTaskIds", "title", "description"]));
  const operationId = stringField(body.operationId, "operationId", { required: true, maxLength: 64 });
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(operationId)) {
    throw new ApiError(400, "INVALID_FIELD", "'operationId' must be a UUID");
  }
  if (!Array.isArray(body.sourceTaskIds) || body.sourceTaskIds.length < 2 || body.sourceTaskIds.length > 50) {
    throw new ApiError(400, "INVALID_FIELD", "'sourceTaskIds' must contain between 2 and 50 task ids");
  }
  const sourceTaskIds = body.sourceTaskIds.map((value, index) => stringField(
    value,
    `sourceTaskIds[${index}]`,
    { required: true, maxLength: 128 },
  ));
  if (new Set(sourceTaskIds).size !== sourceTaskIds.length || sourceTaskIds.includes(operationId)) {
    throw new ApiError(400, "INVALID_FIELD", "'sourceTaskIds' must be unique and cannot contain operationId");
  }
  return {
    operationId,
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (projectRoute) {
        ...
        validateProjectId(projectId);
        if (request.method === "PATCH") {
          const input = parseProjectPatch(await readJson(request));
          const project = database.renameProject(projectId, input.name);
      ...
      if (pathname === "/api/tasks/merge") {
        if (request.method !== "POST") return methodNotAllowed(response, ["POST"]);
        if ([...url.searchParams.keys()].length > 0) {
          throw new ApiError(400, "UNKNOWN_QUERY_PARAMETER", "Task merge does not accept query parameters");
        }
        const result = database.mergeTasks({
          ...parseTaskMerge(await readJson(request)),
          actor: actorFromRequest(request),
        });
        if (!result.replayed) {
          events.emit("task.created", { task: result.task });
          for (const task of result.sourceTasks) events.emit("task.archived", { task });
          events.emit("task.relation.updated", { task: result.task, relatedTasks: result.sourceTasks });
        }
