...
export class TaskboardDatabase {
  ...
  deleteProject(id) {
    ...
    if (id === DEFAULT_PROJECT_ID || id === JIRA_PROJECT_ID) {
      throw new ApiError(403, "PROJECT_DELETE_FORBIDDEN", "System projects cannot be deleted");
    }
    const result = this.database.prepare(`
  ...
  createTask(input) {
    ...
    try {
      ...
      this.database.exec("ROLLBACK TO SAVEPOINT create_task; RELEASE SAVEPOINT create_task");
      throw error;
    }
  }
  ...
  mergeTasks(input) {
    ...
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const existing = this.database.prepare("SELECT * FROM tasks WHERE id = ?").get(input.operationId);
      if (existing) {
        const target = taskFromRow(existing);
        ...
        const relatedIds = new Set(relations.map((relation) => (
          relation.source_task_id === target.id ? relation.target_task_id : relation.source_task_id
        )));
        if (
          target.projectId !== input.projectId
          || target.title !== input.title
          || sourceTaskIds.some((id) => !relatedIds.has(id))
        ) {
          throw new ApiError(409, "MERGE_OPERATION_CONFLICT", "Merge operation id is already used by another task");
        }
        this.database.exec("COMMIT");
        return {
          task: this.getTask(target.id),
          sourceTasks: sourceTaskIds.map((id) => this.#requireTask(id)),
          replayed: true,
        };
      }

      const sourceTasks = sourceTaskIds.map((id) => this.#requireTaskRecord(id));
      if (sourceTasks.length < 2) {
        throw new ApiError(400, "MERGE_REQUIRES_MULTIPLE_TASKS", "At least two source tasks are required");
      }
      for (const task of sourceTasks) {
        if (task.projectId !== input.projectId) {
          throw new ApiError(400, "MERGE_PROJECT_MISMATCH", "Source tasks must belong to the selected project");
        }
        if (task.archivedAt !== null || (task.status !== "backlog" && task.status !== "todo")) {
          throw new ApiError(409, "MERGE_SOURCE_UNAVAILABLE", "Source tasks must be unarchived backlog or todo tasks");
        }
        if (task.source !== "local") {
          throw new ApiError(409, "MERGE_SOURCE_UNAVAILABLE", "Jira tasks cannot be merged and archived locally");
        }
      }
      ...
      const comments = this.database.prepare(`
        ...
        WHERE task_id IN (${placeholders})
          ...
          AND author_id != 'paseo-agent'
        ORDER BY created_at, id
      `).all(...sourceTaskIds).map(commentFromRow);
      const attachments = this.database.prepare(`
        SELECT * FROM attachments
        WHERE task_id IN (${placeholders})
        ORDER BY created_at, id
      `).all(...sourceTaskIds).map(attachmentFromRow);
      const commentsByTask = new Map(sourceTaskIds.map((id) => [id, []]));
      const attachmentsByTask = new Map(sourceTaskIds.map((id) => [id, []]));
      const attachmentsByComment = new Map();
      for (const comment of comments) commentsByTask.get(comment.taskId).push(comment);
      for (const attachment of attachments) {
        if (attachment.commentId) {
          const values = attachmentsByComment.get(attachment.commentId) ?? [];
          values.push(attachment);
          attachmentsByComment.set(attachment.commentId, values);
        } else {
          attachmentsByTask.get(attachment.taskId).push(attachment);
        }
      }
      const attachmentLine = (attachment) => (
        `- [${attachment.filename.replaceAll("\\", "\\\\").replaceAll("]", "\\]")}](/api/attachments/${encodeURIComponent(attachment.id)}/content)`
      );
      const sections = [];
      if (input.description.trim()) sections.push(input.description.trim());
      sections.push("## 合并来源", "以下源想法已归档，并作为相关议题保留完整记录。");
      for (const task of sourceTasks) {
        sections.push(`### ${task.identifier} · ${task.title}`);
        sections.push(task.description.trim() || "（无原始描述）");
        const taskAttachments = attachmentsByTask.get(task.id);
        if (taskAttachments.length > 0) {
          sections.push("#### 任务附件", ...taskAttachments.map(attachmentLine));
        }
        const taskComments = commentsByTask.get(task.id);
        if (taskComments.length > 0) {
          sections.push("#### 人工评论");
          for (const comment of taskComments) {
            sections.push(`**${comment.authorName} · ${comment.createdAt}**`, comment.body);
            const commentAttachments = attachmentsByComment.get(comment.id) ?? [];
            if (commentAttachments.length > 0) sections.push(...commentAttachments.map(attachmentLine));
          }
        }
      }
      const description = sections.join("\n\n");
      if (description.length > 100_000) {
        throw new ApiError(413, "MERGED_DESCRIPTION_TOO_LARGE", "Merged task content exceeds 100000 characters");
      }
      const priorityOrder = new Map(["none", "low", "medium", "high", "urgent"].map((value, index) => [value, index]));
      const priority = sourceTasks.reduce((highest, task) => (
        priorityOrder.get(task.priority) > priorityOrder.get(highest) ? task.priority : highest
      ), "none");
      const labels = [...new Set(sourceTasks.flatMap((task) => task.labels))];
      ...
      for (const source of sourceTasks) {
        const [sourceTaskId, targetTaskId] = [task.id, source.id].sort();
        this.database.prepare(`
          INSERT INTO task_relations (
            relation_type, source_task_id, target_task_id, origin, created_at
          ) VALUES ('related', ?, ?, 'manual', ?)
        `).run(sourceTaskId, targetTaskId, timestamp);
        const archived = this.database.prepare(`
          UPDATE tasks
          SET archived_at = ?, version = version + 1, updated_at = ?
          WHERE id = ? AND archived_at IS NULL AND status IN ('backlog', 'todo')
        `).run(timestamp, timestamp, source.id);
        if (archived.changes !== 1) {
          throw new ApiError(409, "MERGE_SOURCE_UNAVAILABLE", "A source task changed before the merge completed");
        }
        this.#recordTaskActivity(
          source.id,
          input.actor,
          [{ field: "archivedAt", before: null, after: timestamp }],
          timestamp,
        );
        this.#recordTaskActivity(
          task.id,
          input.actor,
          [{ field: "relation", before: null, after: relationActivityValue("related", source) }],
          timestamp,
        );
      }
      this.database.exec("COMMIT");
